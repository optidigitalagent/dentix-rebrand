...
for (const [engineName, engine] of Object.entries({chromium,webkit}).filter(([name]) => !process.env.DENTIX_QA_ENGINE || name === process.env.DENTIX_QA_ENGINE)) {
 ...
 try {
  for (const [width,height] of sizes) {
   ...
   if(width===390){await page.locator('footer.footer').scrollIntoViewIfNeeded();await page.waitForTimeout(100);assert.equal(await page.locator('.mobile-call-bar').count(),0);await page.locator('.lead-form').first().scrollIntoViewIfNeeded();await page.screenshot({path:`${output}/${engineName}-price390.png`});}
