 ...
 try {
  for (const [width,height] of sizes) {
   ...
   await page.route('**/api/public/**', async route => {
    ...
    else if (url.pathname.endsWith('/catalog')) data = {mode:'LIVE_REQUESTS_READY',testOnly:false,timezone:'Europe/Kyiv',requestDurationMinutes:60,minDate:'2026-10-01',maxDate:'2026-10-30',consentVersion:'fixture-v1',services:[{id:'service',name:'Ізольована послуга',category:'QA fixture'}],doctors:[{id:'doctor',name:'Ізольований лікар',role_label:'QA fixture'}],doctorServices:[{doctor_id:'doctor',service_id:'service',active:true,public_bookable:true,booking_mode:'DIRECT_SLOT',reservation_duration_minutes:60,buffer_before_minutes:0,buffer_after_minutes:0,consultation_service_id:null,scheduled_service_id:'service',scheduled_service_name:'Ізольована послуга'}]};
    else if (url.pathname.endsWith('/availability')) data={date:'2026-10-01',slots:[{startsAt,endsAt,localStart:'13:00',localEnd:'14:00'}]};
   ...
   if(width===390){await page.locator('footer.footer').scrollIntoViewIfNeeded();await page.waitForTimeout(100);assert.equal(await page.locator('.mobile-call-bar').count(),0);await page.locator('.lead-form').first().scrollIntoViewIfNeeded();await page.screenshot({path:`docs/qa/mobile-intake-2026-09-07/${engineName}-price390.png`});}
